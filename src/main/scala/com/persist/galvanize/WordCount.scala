...
object Util {
  ...
  val space = """[^\p{IsAlphabetic}]+""".r

  def getAllWords(s: String): Seq[String] = {
    space.split(s)
    .filter(_ != "")

  }

  def getWords(s: String): Seq[String] = {
    getAllWords(s)
    .filter(_.size > 1)
    .map(_.toLowerCase)
    .filter(!stopWords.contains(_))
  }
  def saveList[K, V](list: List[(K, V)], fname: String): Unit = {
    val f = new File(fname)
    f.delete()
    val p = new PrintWriter(fname)
    list.foreach(pair => p.write(pair.toString + "\n"))
    p.close
  }
...
object Example {
  ...
  val sherlockFile = "data/sherlock.txt"
  val out = "target/sherlockCnt.txt"

  def countWords(): Unit = {
    val input = Source.fromFile(sherlockFile).getLines()
    val words = input.flatMap(getWords)
    val counts = words
    .toList
    .groupBy(word => word)
    .map { case (word, list) => (word, list.length) }.toList
    .map { case (word, cnt) => (cnt, word) }
    .sortBy { case (cnt, word) => -cnt }
    saveList(counts, out)
    println(s"scala ${counts.size}")
  }
}
